...
| Unlock gate flag | The one environment flag that disarms the browser gate, and its once-per-process warning — kept free of `node:` builtins so Next can bundle it for the edge runtime too | `packages/core/src/unlock-gate/MODULE.md` |
| Web component test harness | The vitest + React Testing Library setup, and the alias plugin that makes vitest resolve exactly as webpack and the tsx hook do | `packages/web/src/testing/MODULE.md` |
...
- Approval gate: web `/api/approvals*` panel or CLI `approvals`/run-action prompt -> `core/actions/approval` apply/reject by queue row id (claimed, applied and resolved one chunk at a time) -> `core/gmail/operations` writes. Queue rows are written only after their parent `action_results` row exists; proposals identical to a still-pending row are skipped best-effort (a check-then-insert, so concurrent runs can still produce duplicates); the claim is per chunk, so a crash strands at most one chunk of THAT CALL in `applying` while every id that call had not reached stays `pending` — **the bound is per call, not per batch**, because nothing serializes two concurrent applies over one batch and they leapfrog (A claims ids 1-10 and calls Gmail; B loses those, continues, and claims 11-20 while A is in flight), so one crash with two in-flight callers strands two chunks; resolved rows are pruned on the `retention.approvalQueueDays` window; and rows a crash left in `applying` are checked against Gmail's current label state first (`verifyStrandedApplyingOperations`, gated on a cheap DB read so nothing stale means zero Gmail calls), which resolves most of them without a human; only what that check could not answer (SIX reasons: message gone, this account's credentials not working, the check itself failing, an operation this build cannot classify, an unscoped `""`-account row it can never trust as `applied`, or a row the pass ran out of its time budget before reaching) is reported by `getStaleApplyingOperations()` for the user to adjudicate, surfaced by `GET /api/approvals/stranded` + `POST .../verify` + `StrandedOperationsPanel` and by `email-agent approvals stranded [--review]` (which now runs the same check inline before listing). Adjudication (`adjudicateStrandedOperations`) is the FALLBACK: it records what the USER reports finding in Gmail for exactly that residual; there is still no retry for either path, because a row is claimed before Gmail is called and no check — automated or human — can distinguish "we did this" from "it was already like that". Gmail is never mutated by an action run without the user's explicit approval, unless they opted into `gmail.autoApplyActions` and accepted its warnings (`gmail.autoApplyAcknowledged`), in which case the runner applies the queued batch immediately.
...
- Web mutations: browser-origin writes must pass `modules/api` mutation-origin checks before touching Gmail, DB, config, or user action files; mail-returning reads must pass `readGuardResponse`. Since 2026-08-22 both guards additionally require TWO factors: a session cookie obtained by redeeming a one-time unlock token printed by `email-agent serve`/`email-agent unlock`, AND an origin-scoped binding value echoed in a request header. The second factor exists because cookies are NOT scoped by TCP port (RFC 6265 §8.5), so a hostile process holding any other loopback port receives the cookie on a top-level navigation and could replay it; `localStorage` IS scoped by origin including port, so a replayed cookie alone now answers 401 `binding-required` (`core/config/session.ts`, mint/exchange/validate; `modules/api/validation.ts`'s `sessionViolation`, enforcement; `(app)/layout.tsx`, the page-level UX gate) — `EMAIL_AGENT_ALLOW_REMOTE_MUTATIONS=1` bypasses this the same way it bypasses the header checks.
