...
- Approval gate: web `/api/approvals*` panel or CLI `approvals`/run-action prompt -> `core/actions/approval` apply/reject by queue row id (claimed, then applied and resolved in chunks) -> `core/gmail/operations` writes. Queue rows are written only after their parent `action_results` row exists, proposals identical to a still-pending row are skipped, resolved rows are pruned on the `retention.approvalQueueDays` window, and rows a crash left in `applying` are reported by `getStaleApplyingOperations()` for the user to adjudicate. Gmail is never mutated by an action run without the user's explicit approval, unless they opted into `gmail.autoApplyActions` and accepted its warnings (`gmail.autoApplyAcknowledged`), in which case the runner applies the queued batch immediately.
