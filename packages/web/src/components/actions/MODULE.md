...
- product/module functionality: Let users browse built-ins, create/edit user actions, save generated source, run actions, and review/approve queued Gmail changes (`approval-panel.tsx`: per-email checkboxes, click-to-read dialog, apply/reject).
- scope boundaries: Does not write files directly or execute actions; all persistence/execution goes through API routes.
- connected modules/submodules: `hooks/use-actions`, `hooks/use-approvals`, `hooks/use-action-chat`, `store/action-chat-store`, `/api/actions*`, `/api/approvals*`, core actions.
