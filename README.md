...
- **Node.js** >= 22.18 (needed for unflagged TypeScript type stripping, which the native loader uses to import user-created `.action.ts` files; also covers `process.loadEnvFile`, used to load the root `.env`)
...
| Layer | Technology |
|---|---|
| Runtime | Node.js 22.18+, TypeScript 5.8 |
