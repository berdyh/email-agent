...
- `packages/core/src/db/migrations.ts` — Pure column-probe/row-projection helpers for the drop+recreate migrations in `connection.ts`
...
- Adding a column to `pending_operations` (or any audit table): read every row, drop, recreate, **re-insert** via `db/migrations.ts` (`missingColumns` + `projectRowsToSchema`). Dropping the table takes the applied/rejected audit trail with it — that is what the old `claimToken` migration did while warning only about "queued (unapproved)" changes
...
### Config
- `loadSettings()` caches by file identity (path + mtime + size) and re-reads when any changes. Do NOT restore an unconditional cache: `gmail.autoApplyActions` is the kill switch for unattended Gmail mutation, and a stale copy keeps auto-applying after the user turns it off
