...
- **Applying approved operations is serial, and stays that way for now.** `applyOperations()` awaits one Gmail round trip per operation and returns exactly one outcome per input operation IN ORDER, because `toOperationOutcomes` pairs them positionally with the claimed rows. `messages.batchModify` is REJECTED rather than pending: it reports no per-message result (N rows would collapse onto one all-or-nothing status — the ambiguity that mapping deliberately fails closed on), it cannot express `trash` (`batchDelete` is PERMANENT deletion and must never be substituted), and only identical (account, addLabelIds, removeLabelIds) tuples could share a call. A bounded pool is deferred: it would have to partition by (account, message) and stay serial inside a partition (several pending operations can target one message, and serial order is the order the user reviewed), it must wait for retry/backoff (a failure here is TERMINAL — a `failed` row is not `pending`, so an approved change is dropped until the action is re-run), and it must live strictly INSIDE one chunk, since spanning chunks would put two chunks in flight and undo the reason the claim sits in the loop.
